...
uint8_t i2c_add_job(I2C_Data* data) {
	...
	if (i2c_status[I2C_getNum(data->device)].i2c_initialized == 0) {
		return 1;
	}
...
void I2C_Handler(LPC_I2C_T *I2Cx) {
	...
	case I2C_I2STAT_M_TX_RESTART:
		...
		if ((*I2C_active[I2C_num]).dir == 0) {
			...
			I2Cx->CONCLR = I2C_I2CONCLR_SIC;
			return;
		} else if (I2C_active[I2C_num]->dir == 1
				&& I2C_active[I2C_num]->rx_count
						!= I2C_active[I2C_num]->rx_size) {
			...
			I2C_active[I2C_num]->job_done = 1; /*wenn es nichts mehr zu übertragen gibt ist der job erledigt */
			active_job_done[I2C_num] = 1;
			if (!I2C_RB_empty(&I2C_buffer[I2C_num]))
				I2C_send(I2Cx);
			return;
		}
	...
	case I2C_I2STAT_M_TX_DAT_ACK:
		...
		if ((*I2C_active[I2C_num]).dir == 0) {
			if ((*I2C_active[I2C_num]).tx_size
					> (*I2C_active[I2C_num]).tx_count) {
				I2Cx->DAT = *(uint8_t *) (I2C_active[I2C_num]->tx_data
						+ I2C_active[I2C_num]->tx_count); /*pointer auf tx daten wird um tx_count verlängert, dann werden die daten ausgelesen */
				I2C_active[I2C_num]->tx_count++; /*nächsten daten gesendet, daher count erhöhen */
				I2Cx->CONCLR = I2C_I2CONCLR_SIC; /*reset interrupt */
				return;
			} else if ((*I2C_active[I2C_num]).rx_size
					> (*I2C_active[I2C_num]).rx_count) {
	...
	case I2C_I2STAT_M_RX_DAT_ACK:
		...
		if (DEBUG_MODE == 0
			...
			*((uint8_t*) (I2C_active[I2C_num]->rx_data
					+ I2C_active[I2C_num]->rx_count)) = I2Cx->DAT; /*daten speichern */
			I2C_active[I2C_num]->rx_count++;
		} else {
			I2C_active[I2C_num]->error = I2C_ERROR_RX_OVERFLOW;
		}

		if (I2C_active[I2C_num]->rx_count
				< (I2C_active[I2C_num]->rx_size - 1)) {
	...
	case I2C_I2STAT_M_RX_DAT_NACK:
		...
		I2C_active[I2C_num]->job_done = 1; /*wenn es nichts mehr zu empfangen gibt ist der job erledigt */
		active_job_done[I2C_num] = 1;
		if (!I2C_RB_empty(&I2C_buffer[I2C_num]))
			I2C_send(I2Cx); /*starte nächsten Job, sofern vorhanden. */
		return;
	...
	case I2C_I2STAT_M_RX_ARB_LOST: /* Arbitration lost */
		...
		i2c_status[I2C_num].i2c_error_counter++;
	...
	default:
		...
		i2c_status[I2C_num].i2c_interrupt_handler_error = 1;
		i2c_status[I2C_num].i2c_error_counter++;
